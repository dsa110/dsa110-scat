...
#    p0[:, 1] = np.clip(p0[:, 1], -20, 20)  # spectral_index between -10 and 10
...
#):
#    """
#    Parallel MCMC with emcee + HDFBackend.
#
#    - nprocs: number of worker processes.
#    - filename: where to stream the chain (in HDF5).
#    """
#    ndim = len(initial_params)
#    if nwalkers is None:
#        nwalkers = max(10 * ndim, 50)
#
#    # 1) Set up the HDF backend (clears any existing file):
#    backend = backends.HDFBackend(filename)
#    backend.reset(nwalkers, ndim)
#
#    # 2) Initialize walker positions within prior bounds:
#    p0 = np.empty((nwalkers, ndim))
#    for i in range(nwalkers):
#        # c0
#        p0[i, 0] = np.random.uniform(*prior_bounds['c0'])
#        # t0
#        p0[i, 1] = np.random.uniform(*prior_bounds['t0'])
#        # spectral_index
#        p0[i, 2] = np.random.uniform(*prior_bounds['spectral_index'])
#        idx = 3
#        if model_type == 'model1':
#            p0[i, idx] = np.random.uniform(*prior_bounds['zeta'])
#        elif model_type == 'model2':
#            p0[i, idx] = np.random.uniform(*prior_bounds['tau_1GHz'])
#        elif model_type == 'model3':
#            p0[i, idx]     = np.random.uniform(*prior_bounds['zeta'])
#            p0[i, idx + 1] = np.random.uniform(*prior_bounds['tau_1GHz'])
#
...
#            ndim,
#            model.log_posterior,
#            args=(prior_bounds, model_type),
#            pool=pool,
#            backend=backend
#        )
#
#        # 4) Run MCMC and stream to disk
#        sampler.run_mcmc(p0, nsteps, progress=True)
#
#    return sampler
