...
----------------------------------------
1. **Full two‑screen delay geometry** – includes cross‑term −2θ₁·θ₂D₁₂ and
...
class Scintillator:
    ...
    def _single_screen_delays(self):
        s = self.screen1
        theta2 = (s.xy**2).sum(axis=1)  # |θ|² per image
        # Eq. (2.2) specialised
    ...
    def _double_screen_delays(self):
        ...
        θ1_sq = (θ1**2).sum(axis=1)                      # (N1,)
        θ2_sq = (θ2**2).sum(axis=1)                      # (N2,)
